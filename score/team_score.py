...
class ELOScoreDataSetGenerator:
    ...
    def generate_random_team_2(self) -> TeamScoreData:
    ...
    def generate_random_team(self) -> TeamScoreData:
        # 英雄和技能的不均匀分布概率，写死配置
        # 假设HERO_POOL和SKILL_POOL为list，长度分别为40和18
        # 英雄概率分布（示例：前5个英雄概率高，其余低）
        hero_probs = [0.1, 0.1, 0.08, 0.08, 0.08] + [0.56/35]*35  # 总和为1
        # 技能概率分布（示例：前3个技能概率高，其余低）
        skill_probs = [0.1, 0.2, 0.5] + [0.65/15]*15  # 总和为1

        # 按照概率分布采样英雄，保证不重复
        heroes = []
        available_heroes = HERO_POOL.copy()
        available_probs = hero_probs.copy()
        for _ in range(TEAM_SIZE):
            chosen_idx = random.choices(range(len(available_heroes)), weights=available_probs, k=1)[0]
            heroes.append(available_heroes[chosen_idx])
            # 移除已选英雄及其概率
            del available_heroes[chosen_idx]
            del available_probs[chosen_idx]
            # 重新归一化概率
        ...
        # 按照概率分布采样技能（允许重复）
        skills = random.choices(SKILL_POOL, weights=skill_probs, k=TEAM_SIZE)
...
class TeamScoreTrainer:
    ...
    def __init__(self, train_teams: List[TeamScoreData], valid_teams: List[TeamScoreData], log_dir: str = "./logs/team_score"):
        ...
        self.train_dataset = TeamScoreDataset(train_teams)
    ...
    def train(self, epochs: int = 200, save_path: str = "./models/team_score_model.pth"):
        """训练模型"""
...
def main():
    ...
    data_generator.generate_elo_dataset(init_count=200, rounds=2000, battles_per_round=40,add_team_num_per_round=30)
    #data_generator.generate_elo_dataset(init_count=50, rounds=1, battles_per_round=10, add_team_num_per_round=10)
    data_generator.save_dataset("./elo_score_dataset_unbalance.json")
    ...
    trainer.train(epochs=500, save_path="./models/team_score_model.pth")
    
    # 评估
    predictor = TeamScorePredictor(model_path="./models/team_score_model.pth")
