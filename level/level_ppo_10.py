...
class LevelGenerationEnv(gym.Env):
    ...
    def step(self, action):
        ...
        elif self.current_pos % 4 == 0:
            potential = self.calculate_final_reward()
            reward = 1.0*(potential - self.last_potential) 
            reward = np.clip(reward, -0.2, 0.2) # 避免过于震荡
...
class LevelGenerationTrainer:
    ...
    def train(self, total_timesteps=1500000, moves_range=None, score_range=None):
        ...
        model.learn(
            ...
            tb_log_name="level_ppo_10"
        )
        
        model.save("models/level_generator_10")

class LevelGenerator:
    def __init__(self, model_path="models/level_generator_10"):
...
if __name__ == "__main__":    
    ...
    #evaluator = LevelEvaluator("models/level_generator_10")
    #print(evaluator.evaluate(moves_range, score_range))
    

    generator = LevelGenerator("models/level_generator_10")
