...
public class PoseAnimatorWithConstraints : MonoBehaviour
{
    Animator animator;
    ...
    public ModelConfig modelConfig = new Role1ModelConfig(); 
    ...
    public void PlayFrame(PoseFrame poseFrame)
    {
        foreach (BasePoseBone poseBone in poseBones)
        {
            ...
            // 应用约束
            BoneConstraints boneConstraints = modelConfig.GetBoneConstraints(poseBone.boneType);
            rotation = ApplyConstraints(poseBone, rotation, boneConstraints);
    ...
    public Quaternion ApplyConstraints(BasePoseBone poseBone, Quaternion originalRotation, BoneConstraints constraints)
    {
        // 将世界坐标旋转转换为相对于父节点的本地旋转
        Quaternion localRotation;
        Transform parent = poseBone.bone.parent;
        if (parent != null)
        {
            localRotation = Quaternion.Inverse(parent.rotation) * originalRotation;
        }
        else
        {
        ...
        // 应用旋转限制
        eulerAngles.x = Mathf.Clamp(eulerAngles.x, constraints.minRotation.x, constraints.maxRotation.x);
        eulerAngles.y = Mathf.Clamp(eulerAngles.y, constraints.minRotation.y, constraints.maxRotation.y);
        eulerAngles.z = Mathf.Clamp(eulerAngles.z, constraints.minRotation.z, constraints.maxRotation.z);

        // 将受限制的本地旋转转换回世界坐标
        Quaternion constrainedLocalRotation = Quaternion.Euler(eulerAngles);
        if (parent != null)
        {
            return parent.rotation * constrainedLocalRotation;
        }
        else
        {
            return constrainedLocalRotation;
        }
    }
