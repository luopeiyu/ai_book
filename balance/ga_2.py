...
class CoachManager:
    ...
    def __init__(self, initial_coaches: List[List[Tuple[int, int]]], 
                 replace_count: int = 1,
                 max_coaches: int = 30, 
                 update_interval: int = 10):
        
        if len(initial_coaches) == 0:
            self.coaches = [TeamIndividual.generate_random_team() for _ in range(max_coaches)]
        else:
    ...
    def should_update(self, generation: int) -> bool:
        """检查是否需要更新教练队伍"""
        return generation - self.last_update >= self.update_interval
    
    def update_coach(self, generation: int, population: List) -> None:
        """更新教练队伍"""
        if not self.should_update(generation):
            return
        ...
        for individual in sorted_population:
            if added_count >= self.replace_count:
                break
            
            # 检查是否已存在相同的教练队伍
            if individual not in self.coaches:
                # 随机调换英雄顺序
                individual.team_config = random.sample(individual.team_config, len(individual.team_config))
                self.coaches.append(copy.deepcopy(individual))
                added_count += 1
        
        # 如果超过最大数量，移除最早添加的教练队伍
        ...
        print(f"更新教练队伍：添加 {added_count} 个新教练，当前教练数量: {len(self.coaches)}")
        print(f"  教练队伍: {[str(coach.team_config) for coach in self.coaches]}")
...
class TeamGeneticAlgorithm:
    ...
    def __init__(self, 
        ...
        self.coach_manager = CoachManager(initial_coach_teams, replace_count=replace_count, max_coaches=max_coaches, update_interval=update_interval)
    ...
    def calculate_fitness_batch(self, individuals: List[TeamIndividual]) -> None:
        ...
        """
        # 构造参数
        coach_teams = self.coach_manager.coaches
        teams1 = [individual.team_config for individual in individuals]
        teams2 = [coach_team.team_config for coach_team in coach_teams]
        ...
        for i, individual in enumerate(individuals):
            individual.fitness = calculate_individual_fitness(individual_results[i])
                
    
    def calculate_individual_fitness_simple(self, battle_results: List[Dict]) -> float:
        """通过已有战斗结果 计算单个个体的适应度"""
        ...
        # 胜率作为适应度
        fitness = wins / total_battles if total_battles > 0 else 0.0
        return fitness
    ...
    def evolve(self) -> Tuple[List[TeamIndividual], List[float]]:
        ...
        print(f"教练队伍数量: {len(self.coach_manager.coaches)}")
        ...
        for generation in range(self.generations):
            ...
            if generation == self.generations - 1:
                sorted_population = sorted(population, key=lambda x: x.fitness, reverse=True)
                return sorted_population, self.generation_stats
                  
            # 更新教练队伍
            self.coach_manager.update_coach(generation, population)
...
def main():
    ...
    # 创建遗传算法实例
    ga2 = TeamGeneticAlgorithm(
        client=client,
        population_size=20,  # 减小种群大小以加快测试
        generations=100,      # 减少代数以加快测试
        mutation_rate=0.2,
        crossover_rate=0.8,
        elite_rate=0.2,
        initial_coach_teams=coach_teams,
    ...
    calculate_win_rate_matrix(final_population, ga.coach_manager.coaches, client)
