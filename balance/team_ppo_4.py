...
class TeamSelectionEnv(gym.Env):
    ...
    def __init__(self, client: BattleSimulatorClient, coach_teams: List[List[Tuple[int, int]]] = None, hero_pool: List[int] = HERO_POOL):
        ...
        self.hero_pool = hero_pool.copy()
        self.available_heroes = self.get_random_available_heroes()
        self.current_phase = 0  # 当前阶段 0-5 (3个英雄 + 3个技能)
        self.selected_hero = None # 当前选择英雄
        self.step_count = 0     # 当前步数
        self.MAX_STEP = 100    # 最大步数
        
    def get_random_available_heroes(self):
        if random.random() < 0.7:  # 70%概率使用限制池
            pool_size = random.randint(3, len(self.hero_pool))
            return set(random.sample(self.hero_pool, pool_size))
        else:
            return set(self.hero_pool)
...
class DynamicCoachCallback(BaseCallback):
    ...
    def collect_strong_teams_from_model(self, model, coach_teams, count: int = 10) -> List[List[Tuple[int, int]]]:
        ...
        while len(predict_teams) < count and attempts < max_attempts:
            attempts += 1
            isok, predicted_team = predictor.predict(coach_teams, HERO_POOL)
            if not isok:
                continue
...
class TeamSelectionTrainer:
    ...
    def create_env(self):
        ...
        def _init():
            ...
            env = TeamSelectionEnv(local_client, self.coach_teams, HERO_POOL)
            return env
        return _init
    ...
    def train(self, 
              ...
              save_path: str = "./output/ppo_team_selection_4"):
        """训练PPO模型"""
        print("创建训练环境...")
        ...
        model.learn(
            ...
            tb_log_name="ppo_team_selection_4",
            callback=callback
        )
...
class TeamSelectionPredictor:
    ...
    def predict(self, coach_team: List[Tuple[int, int]], hero_pool: List[int] = HERO_POOL) -> int:
...
def evaluate():
    ...
    client = BattleSimulatorClient()
    predictor = TeamSelectionPredictor(model_path="./models/ppo_team_selection_4")
...
def train():
    ...
    trainer.train(
        ...
        learning_rate=3e-4,
        save_path="./models/ppo_team_selection_4"
    )
